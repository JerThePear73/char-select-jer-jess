...
local function command_moveset_guide(msg)
	if msg == "jer" or (msg == "" and is_jer()) then
		...
		djui_chat_message_create("GP (Ground) + B   or   Slide Kick/Dive (Ground) + Z = Cartwheel")
		djui_chat_message_create("Hold Z while in a Cartwheel to increase speed, but don't go too fast")
		djui_chat_message_create("Crouch + B = Burst Slide Kick")
	...
	elseif msg == "jess" or (msg == "" and is_jess()) then
		...
		djui_chat_message_create("Air + B = Galaxy Spin")
...
hook_chat_command("jj-moves", "[jer|jess] - shows moveset guide for the character", command_moveset_guide)
hook_chat_command("jj-passives", "[jer|jess] - shows passives for the character", command_moveset_passives)
